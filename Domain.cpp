...
bool Domain::compute_domain_v1() {  
    ...
    // Verificando se cada tarefa é dominada por um único robô (retorna false se não))
    for (int i = 0; i < n_tasks; i++) {
        std::shared_ptr<PickTask> cur_task = (*this->__pick_tasks_ref)[i];
        std::tuple<std::shared_ptr<PickTask>, std::vector<std::shared_ptr<Robot> > > tpl = this->__domain_list[cur_task->get_id()];
        std::vector<std::shared_ptr<Robot> > cur_robots = std::get<1>(tpl);
        if (cur_robots.size() > 1) return false;
    }
    return true;
}

bool Domain::compute_domain_v2() {
    std::vector<std::shared_ptr<Robot> > best_robots;
    int n_robots = this->__robots_ref->size();
    int n_tasks = this->__pick_tasks_ref->size();
    for (int i = 0; i < n_tasks; i++) {
...
Domain::cost_comparison(std::shared_ptr<Robot> _robot, std::vector<std::shared_ptr<PickTask> > _tasks) {
    ...
    for (int i = 0; i < _tasks.size(); i++) {
        ...
        else if ((cur_cost >= best_cost * (1 - RSE::cost_tolerance)) &&
                (cur_cost <= best_cost * (1 + RSE::cost_tolerance)))
            best_tasks_ref->push_back(cur_task_of_main);
    }
...
Domain::euler_test(std::shared_ptr<Robot> _robot, std::vector<std::shared_ptr<PickTask> > _tasks) {
    ...
    for (int i = 0; i < _tasks.size(); i++) {
        ...
        if (cur_cost < smaller_cost * (1 - RSE::cost_tolerance)) {
            smaller_cost = cur_cost;
            tasks_best_cost_ref->clear();
            tasks_best_cost_ref->push_back(cur_task);
        }
        else if ((cur_cost >= smaller_cost * (1 - RSE::cost_tolerance)) &&
                (cur_cost <= smaller_cost * (1 + RSE::cost_tolerance)))
            tasks_best_cost_ref->push_back(cur_task);
